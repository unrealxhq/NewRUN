...
include('includes/how_about_no.lua')
include('organizer-lib.lua')
include('includes/no_interruptions.lua')
include('includes/spam_protection.lua')
include('includes/common_commands.lua')
include('spellstack/main.lua')
include('includes/spam_protection.lua')
include('includes/auto_set_party_target.lua')
include('Modes.lua')
	rangedDelay = 600
    cast_speed = 1.0
    holdMovement = 0
...
function user_setup()
	...
	send_command ('bind numpadenter send @others //lua r healbot')
	send_command ('bind numpad+ send @others //hb follow Shiraj')
	send_command ('bind numpad- send @others //hb follow dist 2')
	send_command ('bind numpad* send @others //hb follow dist 19')
...
function init_gear_sets()
	...
	sets.MEva = {
		ammo="Staunch Tathlum +1",
		head={ name="Fu. Bandeau +3", augments={'Enhances "Battuta" effect',}},
		body={name="Runeist's Coat +3", priority=4}, 
    ...
    sets.precast.JA['Vallation'] = {
		ammo="Sapience Orb",
		head="Halitus Helm",
		body={name="Runeist's Coat +3", priority=3}, 
		hands="Kurys Gloves",
		legs="Eri. Leg Guards +1",
		feet="Ahosi Leggings",
		neck="Futhark Torque +2",
    ...
    sets.precast.FC = {
		...
		feet="Carmine Greaves"}
			...
			-- Fast Cast: 58% / -23% Enhancing Magic Casting Time / 30% Inspiration
    ...
    sets.precast.WS['Herculean Slash'] = sets.precast.JA['Swipe']
...
function job_post_midcast(spell, action, spellMap, eventArgs)
	...
	elseif spell.english == 'Gambit' then
		send_command('input /party Gambit used on <t> Magic Defense Down -30%;wait96 /p Gambit Off')
	elseif spell.english == 'Rayke' then
		send_command('input /party Rayke used on <t> Magic Resistance Down -30%;wait42 /p Rayke Off')
...
function job_buff_change(status,gain_or_loss)
   if (gain_or_loss) then  
	 ...
	 if status == "Aftermath: Lv.3" then
		send_command('input //timers c "AM3" 181')
		elseif status == "Aftermath: Lv.2" then
		send_command('input //timers c "AM2" 121')
		elseif status == "Aftermath: Lv.1" then
		send_command('input //timers c "AM1" 91')
	 end	
